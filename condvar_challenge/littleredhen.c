...
pthread_cond_t hencond = PTHREAD_COND_INITIALIZER;
pthread_cond_t othercond = PTHREAD_COND_INITIALIZER;
...
void *littleRedHenThread(void *arg) {
  ...
  for (batch = 1; batch <= 6; batch++) {
    ...
    while(numLoaves > 0) {
	pthread_cond_wait(&hencond, &mutex);
    }
    ...
    pthread_cond_broadcast(&othercond);
  }
...
void *otherAnimalThread(void *arg) {
  ...
  while (numLoavesEaten < NUM_LOAVES_TO_EAT) {
	...
	printf("%-20s: Hey, Little Red Hen, make some more bread!\n", name);
        pthread_cond_signal(&hencond);
      }
      pthread_cond_wait(&othercond, &mutex);
    }
    kitchen = 0;
